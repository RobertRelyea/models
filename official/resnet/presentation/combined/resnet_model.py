...
class Model(object):
  ...
  def __init__(self, resnet_size, bottleneck, num_classes, num_filters,
               ...
               dtype=DEFAULT_DTYPE, multi_head=False, num_classes2=0):
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      mmWave_inputs = inputs['mmWave']
      uwb_inputs = inputs['uwb']
      inputs = inputs['image']
      ...
      # uwb_dense1 = tf.layers.dense(uwb_inputs, 64,tf.nn.relu)
      # tf.identity(uwb_dense1, 'uwb_dense1')
      # # Hidden fully connected layer with 256 neurons
      # uwb_dense2 = tf.layers.dense(uwb_dense1, 64,tf.nn.relu)
      # tf.identity(uwb_dense2, 'uwb_dense2')

      # FC 64
      mmWave_dense1 = tf.contrib.layers.fully_connected(inputs=mmWave_inputs, num_outputs=64)
      ...
      if self.multi_head:
          head1_dense = tf.layers.dense(inputs=dense_inputs, units=self.num_classes)
          head1_dense = tf.identity(head1_dense, 'head1_dense')

          head2_dense = tf.layers.dense(inputs=dense_inputs, units=self.num_classes2)
          head2_dense = tf.identity(head2_dense, 'head2_dense')

          return {'head1':head1_dense, 'head2':head2_dense}
      else:
          head1_dense = tf.layers.dense(inputs=dense_inputs, units=self.num_classes)
          head1_dense = tf.identity(head1_dense, 'final_dense')
          return {'head1':head1_dense}
